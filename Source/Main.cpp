...
static void PrintUsage()
{
	std::cout << "Usage: " << AppName << " [options]\n"
			  ...
			  << "                      are built)\n"
			  << "  --renderer=KIND     Which device draws: auto, hardware, software (Vulkan on\n"
			  << "                      llvmpipe), or cpu (gyro's own blitter, which draws no\n"
			  << "                      materials and only the dump backend can present). auto\n"
			  << "                      takes the best one present\n"
			  ...
			  << "  --capture[=DIR]     Allow Ctrl+Alt+Esc S to write the frame on the glass as a PAM\n"
			  << "                      per output. Composite targets are allocated so they can be\n"
			  << "                      read back, which the flag exists to keep off ordinary runs.\n"
			  << "                      The default is ./gyro-captures\n"
			  << "  --chord-pipe[=PATH] Take the chord's verbs from a fifo as well as from the\n"
			  << "                      keyboard, so a script or another terminal can ask for a\n"
			  << "                      capture: echo s > PATH. Anybody who can write to it can\n"
			  << "                      also stop the compositor, so it is off unless asked for.\n"
			  << "                      The default is /run/gyro/chord\n"
			  << "  --no-planes         Never put a client on an overlay plane; composite every\n"
			  << "                      item on the GPU. A debugging flag: it gives the run one\n"
			  << "                      path to the glass, at the cost of what promotion was\n"
			  << "                      saving\n"
			  << "  --display-off=S     Turn every panel off after S seconds with no input, and back\n"
			  << "                      on at the next; the input that wakes them is not delivered.\n"
			  << "                      Drm only. The default never turns them off\n"
