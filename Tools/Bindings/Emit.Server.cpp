...
// The other generated file with no protocol behind it, and it is here for the same reason the
// fault sink is: the generated library links `Core` and `libwayland-server` and nothing else, so
// there is no module in `Source/` for it to live in. What it exists for is written at the top of
// the file it emits — the resource classes promise that a copy of a destroyed object is stale
// rather than dangling, and without this they keep that promise only for the handler's own copy.
EmittedFile EmitWeakHeader()
{
	return EmittedFile{
		.Path = "Wayland/Server/Weak.h",
		.Text = "// The generated server bindings' weak resource. Written by Tools/Bindings. Do not edit.\n"
				"//\n"
				"// This is the second file here with no protocol behind it, and it exists because of a hole in the\n"
				"// promise the resource classes make. A resource is a `wl_resource*` in a one-word wrapper and every\n"
				"// method on one checks that pointer for null — but the check cannot see a resource the client has\n"
				"// destroyed. libwayland frees the block and hands the same memory back for the next object that\n"
				"// client makes, so the pointer stays non-null and starts naming something else entirely. The\n"
				"// handler's own copy is safe, because `WireGone` clears it before `OnGone` runs. A copy anybody\n"
				"// else kept is not.\n"
				"//\n"
				"// So a resource held across a return to the event loop is held in one of these. It watches the\n"
				"// object and goes empty at the instant the object does, which is what makes the null check in\n"
				"// `Release` or `PostError` mean what it reads as.\n"
				"//\n"
				"// **Nothing that lives inside one request needs this.** A resource named by an argument, used, and\n"
				"// dropped before the handler returns cannot outlive its object, and wrapping one would buy nothing\n"
				"// for two list operations.\n"
				"\n"
				"#pragma once\n"
				"\n"
				...
				"namespace Detail\n"
				"{\n"
				"// The half of a weak reference that does not depend on which interface it names, so that the\n"
				"// listener libwayland calls is one function for the whole set rather than one per resource class.\n"
				"//\n"
				"// `Listener` is first and `WeakNotify` casts back through it, which is why this is a plain\n"
				"// standard-layout struct beside the template rather than two members inside it.\n"
				"struct WeakWatch\n"
				"{\n"
				...
				"};\n"
				"\n"
				"inline void WeakNotify(wl_listener* listener, void* /*data*/)\n"
				"{\n"
				"	auto* const watch = reinterpret_cast<WeakWatch*>(listener);\n"
				"\n"
				"	// **Unlinked here rather than left to the destructor**, because the list this link is spliced\n"
				"	// into belongs to the resource libwayland is in the middle of freeing. A watch that outlived its\n"
				"	// object without unlinking would be a destructor writing through two pointers into dead memory,\n"
				"	// which is the same bug one layer down from the one this file is for.\n"
				"	wl_list_remove(&watch->Listener.link);\n"
				"	wl_list_init(&watch->Listener.link);\n"
				"\n"
				"	watch->Resource = nullptr;\n"
				"}\n"
				"\n"
				"inline void WeakWatchBegin(WeakWatch& watch, wl_resource* resource) noexcept\n"
				"{\n"
				"	watch.Resource = resource;\n"
				"	watch.Listener.notify = &WeakNotify;\n"
				"	wl_list_init(&watch.Listener.link);\n"
				"\n"
				"	if (resource != nullptr)\n"
				"	{\n"
				"		wl_resource_add_destroy_listener(resource, &watch.Listener);\n"
				"	}\n"
				"}\n"
				"\n"
				"inline void WeakWatchEnd(WeakWatch& watch) noexcept\n"
				"{\n"
				"	if (watch.Resource == nullptr)\n"
				"	{\n"
				"		return;\n"
				"	}\n"
				"\n"
				...
				"} // namespace Detail\n"
				"\n"
				"// A resource reference that empties itself when the client destroys the object.\n"
				"//\n"
				"// **Copying and moving re-register rather than copy the link**, and that is the whole reason this\n"
				"// is a class and not a resource with a flag beside it. libwayland splices the listener into its\n"
				"// list by address, so a `wl_list` node carried to a new address by an ordinary assignment leaves\n"
				"// two live pointers aimed at where it used to be. Each of the five operations below is therefore a\n"
				"// pair of list operations rather than a word copy — still nothing at the rate requests arrive, and\n"
				...
				"\n"
				"	explicit Weak(T object) noexcept { Detail::WeakWatchBegin(m_Watch, object.WireResource()); }\n"
				"\n"
				"	Weak(const Weak& other) noexcept { Detail::WeakWatchBegin(m_Watch, other.m_Watch.Resource); }\n"
				"\n"
				"	Weak(Weak&& other) noexcept\n"
				"	{\n"
				"		Detail::WeakWatchBegin(m_Watch, other.m_Watch.Resource);\n"
				"		Detail::WeakWatchEnd(other.m_Watch);\n"
				"	}\n"
				"\n"
				...
				"	}\n"
				"\n"
				"	~Weak() { Detail::WeakWatchEnd(m_Watch); }\n"
				"\n"
				"	// Whether the object is still there. False for one that was never given a resource, for one\n"
				"	// given an invalid resource, and for one whose object the client has since destroyed — and a\n"
				"	// caller holding one of these has no reason to tell those three apart.\n"
				"	[[nodiscard]] bool IsValid() const noexcept { return m_Watch.Resource != nullptr; }\n"
				"\n"
				"	// The resource, or an invalid one. What comes back is an ordinary resource and every method on\n"
				"	// it checks for null, so a caller that only wants to send an event need not ask first.\n"
				"	//\n"
				"	// **Live for the expression it is used in and no longer.** Storing what this returns is storing\n"
				"	// the raw pointer again, which is the thing this class exists to stop.\n"
				"	[[nodiscard]] T Get() const noexcept { return T{ m_Watch.Resource }; }\n"
				"\n"
				"	// Stops watching, and is what assigning a default-constructed one would do.\n"
				"	void Reset() noexcept { Detail::WeakWatchEnd(m_Watch); }\n"
				"\n"
				"private:\n"
				"	Detail::WeakWatch m_Watch{};\n"
				"};\n"
				"} // namespace Wayland::Server\n",
	};
}
